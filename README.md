...
Requirements: Windows 10 or 11 x64. Python is not required. The tray is a native Windows window. Portable and installed copies ship MinGit next to the app and keep using that Git when a project folder is moved. If a copy has no bundled Git and Git is already on PATH, AG2C uses the PATH Git and remembers it.
