...
1. **Skill (soft).** Enrollment copies `ag2c-governed-development`, `ag2c-governance-update`, and `ag2c-directory-census` into known agent homes. File changes use the first; document cards the second; 彻查/普查 the third. A model can ignore that. Reading copies live in [docs/skills](skills/README.md). The AI-entry prompt names this AG2C's Skill versions and digests; each paste tells the agent to replace its installed copies if they are not this AG2C's.
