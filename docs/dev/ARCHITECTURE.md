...
The Windows product window is WebView2 through pywebview. It starts the frozen AG2C runtime as a child process on a random `127.0.0.1` port, uses a per-session header token, and shows projects, the file tree, an inspector, and an ops strip. Network calls run on a worker thread. It requires the WebView2 Runtime, does not use PySide/Qt, and does not import 3D plotting modules. The Python runtime serves a local JSON API plus a static `/ui/` page. The server rejects non-local hosts and origins and sends a restrictive content security policy.

The UI can add, inspect, recheck, open, and stop managing projects. A registered folder that no longer exists is listed as unavailable; the window does not run Git on it and does not auto-select it. It cannot edit Policy or evidence. Knowledge cards, unowned directories, stale or abandoned cards, undeclared product checks, and open AI worktrees are flagged in the tree, card list, and inspector.
