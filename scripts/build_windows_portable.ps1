...
$portableRoot = Join-Path $portableBuildRoot 'AutoGovern2Code'
...
if ($LASTEXITCODE -ne 0 -or $packageVersion -ne $Version) {
    throw "Requested portable version $Version does not match package version $packageVersion."
}
...
New-Item -ItemType Directory -Path $portableBuildRoot -Force | Out-Null
...
if ($LASTEXITCODE -ne 0) {
    throw "Portable layout failed with exit code $LASTEXITCODE."
}
Copy-Item -LiteralPath $gitRoot -Destination (Join-Path $portableRoot 'git') -Recurse -Force
if (-not (Test-Path -LiteralPath (Join-Path $portableRoot 'git\cmd\git.exe'))) {
    throw "Portable package is missing its bundled Git."
}

$zip = Join-Path $portableBuildRoot 'AutoGovern2Code-Portable-Windows-x64.zip'
Compress-Archive -LiteralPath $portableRoot -DestinationPath $zip -Force
if (-not (Test-Path -LiteralPath $zip)) {
    throw "Portable zip was not created at $zip."
}
Write-Output $zip
